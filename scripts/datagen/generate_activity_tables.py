...
Usage:
  ...
  python3 scripts/datagen/generate_activity_tables.py --src DIR  # DIR has the five dumps (skips the extractor)
...
from backend.common.activity import MIN_LAB_RESEARCH, MIN_MISSIONS, build_activity_tables
...
def main():
    ...
    ap.add_argument('--src', type=Path, help='dir with the five table dumps (skips the extractor)')
    ...
    doc = build_activity_tables(load_rows(src, 'lab.json'), load_rows(src, 'labtext.json'),
                                ...
                                load_rows(src, 'recipes.json'))
    lab, missions, recipes = doc['lab'], doc['expeditions'], doc['recipe_products']
    ...
    if len(lab) < MIN_LAB_RESEARCH or len(missions) < MIN_MISSIONS:
        print(f'WARNING: fewer rows than expected (lab >= {MIN_LAB_RESEARCH}, expeditions >= {MIN_MISSIONS}) -- stale usmap?')
    ...
    doc = {
        '_comment': 'Generated by scripts/datagen/generate_activity_tables.py from the game\'s '
                    ...
                    'DT_ItemRecipeDataTable. Names, durations and work amounts for what the save says a '
                    'base is doing. Do not edit by hand.',
        **doc,
    }
