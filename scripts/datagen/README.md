...
- **`data/json/activity.json`** -- what the base Activity view needs to name what the save says a
  ...
  names + durations (`DT_CharacterTeamMissionDataTable` + text) and the few recipe ids that are not
  the product's item id (`generate_activity_tables.py`; read by `backend/common/activity.py`).
